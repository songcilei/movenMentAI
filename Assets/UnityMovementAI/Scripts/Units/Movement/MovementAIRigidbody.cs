...
namespace UnityMovementAI
{
    ...
    public class MovementAIRigidbody : MonoBehaviour
    {
        ...
        bool SphereCast(Vector3 dir, out RaycastHit hitInfo, float dist, int layerMask, Vector3 planeNormal = default(Vector3))
        {
             ...
             * 确保我们使用collider的起源为我们的cast(可以是不同的
             *然后是transform.position)。
             ...
             * 同样，如果给定平面法线，则将原点抬高一点点
             *从飞机上避免问题，当给定的dir只是刚刚移动
             *进入平面(这可能发生由于浮点不精确时
             * dir是用叉乘计算的)*/
        ...
        /// 应该用于大多数移动AI代码的位置。对于2D字符，位置将
        /// 在X/Y平面上。对于3D接地单位，位置是在X/Z平面上。对3D
        /// 飞行角色的位置是全3D (X/Y/Z)。
        /// </summary>
        ...
        /// 根据刚体所在的单位类型转换向量。
        /// 如果它是一个2D单位，那么Z组件将被归零。如果它
        /// 是一个接地的3D角色，那么Y组件将被归零。
        /// 如果它是飞行的3D单位，没有改变将对矢量。
        /// </summary>
        /// <param name="v"></param>
        /// <returns></returns>
